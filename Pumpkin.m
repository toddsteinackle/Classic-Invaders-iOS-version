...
- (id)initWithTileLocation:(CGPoint)aLocation {
	...
	if (self != nil) {
		...
		Image *embededSpriteSheetImage = [[pss imageForKey:@"pumpkin_spritesheet.png"] retain];
        spriteSheet = [SpriteSheet spriteSheetForImage:embededSpriteSheetImage sheetKey:@"pumpkin_spritesheet.png" spriteSize:CGSizeMake(40, 40) spacing:0 margin:0];
        ...
        [animation addFrameWithImage:[spriteSheet spriteImageAtCoords:CGPointMake(3, 0)] delay:0.075];
        [animation addFrameWithImage:[spriteSheet spriteImageAtCoords:CGPointMake(4, 0)] delay:0.1];
        [animation addFrameWithImage:[spriteSheet spriteImageAtCoords:CGPointMake(5, 0)] delay:0.075];
        [animation addFrameWithImage:[spriteSheet spriteImageAtCoords:CGPointMake(6, 0)] delay:delay];
        [animation addFrameWithImage:[spriteSheet spriteImageAtCoords:CGPointMake(7, 0)] delay:delay];
        [animation addFrameWithImage:[spriteSheet spriteImageAtCoords:CGPointMake(8, 0)] delay:delay];
        ...
        speed = (float)(RANDOM_0_TO_1() * MAX_SPEED);
...
- (CGRect)movementBounds { 
	...
	return CGRectMake(pixelLocation.x - 19, pixelLocation.y - 19, 35, 38);
}

- (CGRect)collisionBounds {
	CGPoint pixelPos = tileMapPositionToPixelPosition(tileLocation);
	CGRect bounds;
	
	switch (animation.currentFrame) {
		case 0:
			bounds = CGRectMake(pixelPos.x - 19, pixelPos.y - 18, 25, 12);
			break;
		case 1:
			bounds = CGRectMake(pixelPos.x - 16, pixelPos.y - 15, 25, 12);
			break;
		case 2:
			bounds = CGRectMake(pixelPos.x - 14, pixelPos.y - 13, 25, 16);
			break;
		case 3:
			bounds = CGRectMake(pixelPos.x - 12, pixelPos.y - 11, 25, 19);
			break;
		case 4:
			bounds = CGRectMake(pixelPos.x - 10, pixelPos.y - 8, 20, 23);
			break;
		case 5:
			bounds = CGRectMake(pixelPos.x - 8, pixelPos.y - 11, 20, 19);
			break;
		case 6:
			bounds = CGRectMake(pixelPos.x - 8, pixelPos.y - 13, 25, 16);
			break;
		case 7:
			bounds = CGRectMake(pixelPos.x - 8, pixelPos.y - 15, 25, 12);
			break;
		case 8:
			bounds = CGRectMake(pixelPos.x - 6, pixelPos.y - 18, 25, 12);
			break;
		default:
			break;
	}
