...
- (id)initWithTileLocation:(CGPoint)aLocation {
	...
	if (self != nil) {
		...
		Image *embededSpriteSheetImage = [[pss imageForKey:@"spritesheet_witch.png"] retain];
        spriteSheet = [SpriteSheet spriteSheetForImage:embededSpriteSheetImage sheetKey:@"spritesheet_witch.png" spriteSize:CGSizeMake(40, 40) spacing:0 margin:0];
		...
		//        [animation addFrameWithImage:[[spriteSheet getSpriteAtCoords:CGPointMake(1, 0) registerWithIRM:YES] retain] delay:delay];
...
- (CGRect)movementBounds { 
	...
	return CGRectMake(pixelLocation.x - 13, pixelLocation.y - 19, 26, 38);
}

- (CGRect)collisionBounds {
	// Calculate the pixel position and return a CGRect that defines the bounds
	pixelLocation = tileMapPositionToPixelPosition(tileLocation);
	return CGRectMake(pixelLocation.x - 9, pixelLocation.y - 17, 16, 34);
}
