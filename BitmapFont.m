...
- (id)initWithFontImageNamed:(NSString*)aFileName ofType:(NSString*)aFileType controlFile:(NSString*)aControlFile scale:(Scale2f)aScale filter:(GLenum)aFilter {
	self = [self init];
	if (self != nil) {
		...
		image = [[Image alloc] initWithImageNamed:aFileName ofType:aFileType filter:aFilter];
        image.scale = aScale;
...
- (void)renderStringAt:(CGPoint)aPoint text:(NSString*)aText {
	...
	for(int i=0; i<[aText length]; i++) {
		...
		int y = aPoint.y + (commonHeight * yScale) - (charsArray[charID].height + charsArray[charID].yOffset) * yScale;
...
- (void)renderStringJustifiedInFrame:(CGRect)aRect justification:(int)aJustification text:(NSString*)aText {
	...
	switch (aJustification) {
		case BitmapFontJustification_TopLeft:
			...
			point.y = aRect.origin.y + (aRect.size.height - textHeight) - (commonHeight - textHeight);
			break;
		case BitmapFontJustification_MiddleLeft:
			point.x = aRect.origin.x;
			point.y = aRect.origin.y + ((aRect.size.height - textHeight) / 2) - (commonHeight - textHeight);		
			break;
		case BitmapFontJustification_BottomLeft:
			point.x = aRect.origin.x;
			point.y = aRect.origin.y - (commonHeight - textHeight);
			break;
...
- (void)parseCharacterDefinition:(NSString*)line {
	...
	// Character ID
	propertyValue = [nse nextObject];
    int charID = [propertyValue intValue] - 32;
    charsArray[charID].charID = [propertyValue intValue];
	// Character x
	propertyValue = [nse nextObject];
    charsArray[charID].x = [propertyValue intValue];
	// Character y
	propertyValue = [nse nextObject];
    charsArray[charID].y = [propertyValue intValue];
	// Character width
	propertyValue = [nse nextObject];
    charsArray[charID].width = [propertyValue intValue];
	// Character height
	propertyValue = [nse nextObject];
    charsArray[charID].height = [propertyValue intValue];
	// Character xoffset
	propertyValue = [nse nextObject];
    charsArray[charID].xOffset = [propertyValue intValue];
	// Character yoffset
	propertyValue = [nse nextObject];
    charsArray[charID].yOffset = [propertyValue intValue];
	// Character xadvance
	propertyValue = [nse nextObject];
    charsArray[charID].xAdvance = [propertyValue intValue];
