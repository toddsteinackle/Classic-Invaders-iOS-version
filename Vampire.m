...
//  SLQTSOR
//
//  Created by Mike Daley on 06/11/2009.
...
- (id)initWithTileLocation:(CGPoint)aLocation {
	...
	if (self != nil) {
		...
		Image *embededSpriteSheetImage = [[pss imageForKey:@"spritesheet_vampire.png"] retain];
        spriteSheet = [SpriteSheet spriteSheetForImage:embededSpriteSheetImage sheetKey:@"spritesheet_vampire.png" spriteSize:CGSizeMake(40, 40) spacing:0 margin:0];
		
        animation = [[Animation alloc] init];
		[animation addFrameWithImage:[spriteSheet spriteImageAtCoords:CGPointMake(0, 0)] delay:4.0];
        [animation addFrameWithImage:[spriteSheet spriteImageAtCoords:CGPointMake(1, 0)] delay:0.1];
		[animation addFrameWithImage:[spriteSheet spriteImageAtCoords:CGPointMake(2, 0)] delay:0.6];
...
- (CGRect)collisionBounds {
	// Calculate the pixel position and return a CGRect that defines the bounds
	pixelLocation = tileMapPositionToPixelPosition(tileLocation);
	return CGRectMake(pixelLocation.x - 12, pixelLocation.y - 17, 24, 34);
}
