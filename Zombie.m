...
- (id)initWithTileLocation:(CGPoint)aLocation {
	...
	if (self != nil) {
		...
		Image *embededSpriteSheetImage = [[pss imageForKey:@"spritesheet_zombie.png"] retain];
        spriteSheet = [SpriteSheet spriteSheetForImage:embededSpriteSheetImage sheetKey:@"spritesheet_zombie.png" spriteSize:CGSizeMake(40, 40) spacing:0 margin:0];
...
- (CGRect)movementBounds { 
	...
	return CGRectMake(pixelLocation.x - 12, pixelLocation.y - 19, 24, 38);
}

- (CGRect)collisionBounds {
	// Calculate the pixel position and return a CGRect that defines the bounds
	pixelLocation = tileMapPositionToPixelPosition(tileLocation);
	return CGRectMake(pixelLocation.x - 8, pixelLocation.y - 17, 19, 34);
}
