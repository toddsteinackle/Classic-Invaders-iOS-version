...
- (id)initParticleEmitterWithFile:(NSString*)aFileName ofType:(NSString*)aFileType {
		...
		if (self != nil) {
			...
			TBXML *particleXML = [[TBXML alloc] initWithXMLFile:aFileName fileExtension:aFileType];
...
- (void)renderParticles {
    ...
    GLint textureName = (GLuint)[[texture texture] name_];
...
- (void)parseParticleConfig:(TBXML*)aConfig {
	...
	TBXMLElement *element = [aConfig childElementNamed:@"texture" parentElement:rootXMLElement];
	if (element) {
		NSString *fileName = [aConfig valueOfAttributeNamed:@"name" forElement:element];
		NSString *fileType = [aConfig valueOfAttributeNamed:@"type" forElement:element];

		if (fileName) {
			// Create a new texture which is going to be used as the texture for the point sprites
			texture = [[Image alloc] initWithImageNamed:fileName ofType:fileType filter:GL_LINEAR];
		}
	}
