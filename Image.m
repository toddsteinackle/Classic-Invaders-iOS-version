...
- (void)initializeImage:(NSString*)aName ofType:(NSString*)aType filter:(GLenum)aFilter;
...
- (id)initWithImageNamed:(NSString*)aName ofType:(NSString*)aType filter:(GLenum)aFilter {
	...
	if (self != nil) {
        ...
        [self initializeImage:aName ofType:aType filter:aFilter];
...
- (Image*)subImageInRect:(CGRect)aRect {
    ...
    Image *subImage = [[Image alloc] initWithImageNamed:imageFileName ofType:imageFileType filter:minMagFilter subTexture:aRect];
...
- (void)render {
    ...
    if (dirty) {
        ...
        if(flipVertically) {
            ...
            translateMatrix(matrix, CGPointMake(-imageSize.width, 0));
        }

        if(flipHorizontally) {
            scaleMatrix(matrix, Scale2fMake(1, -1));
            translateMatrix(matrix, CGPointMake(0, -imageSize.height));
        }
...
- (void)initializeImage:(NSString*)aImageName ofType:(NSString*)aType filter:(GLenum)aFilter {
	...
	self.texture = [textureManager textureWithFileName:imageFileName ofType:imageFileType filter:aFilter];
	...
	// Set the default rotation point
