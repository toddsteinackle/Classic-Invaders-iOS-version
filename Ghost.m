...
- (id)initWithTileLocation:(CGPoint)aLocation {
	...
	if (self != nil) {
		...
		Image *embededSpriteSheetImage = [[pss imageForKey:@"ghost_spritesheet.png"] retain];

		// We want the ghosts to be transparent, so adjust the color of the spritesheet
		embededSpriteSheetImage.color = Color4fMake(1, 1, 1, 0.6f);
        
		spriteSheet = [SpriteSheet spriteSheetForImage:embededSpriteSheetImage sheetKey:@"ghost_spritesheet.png" spriteSize:CGSizeMake(40, 40) spacing:0 margin:0];
        ...
        animation.state = kAnimationType_Repeating;
        animation.type = kAnimationState_Running;
...
- (CGRect)movementBounds { 
	...
	return CGRectMake(pixelLocation.x - 19, pixelLocation.y - 19, 38, 38);
}
