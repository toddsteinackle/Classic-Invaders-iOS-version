    ...
    if(self != nil) {
		...
		switch (type) {
			...
			case kDoorH_StoneRed:
				openDoor = [pss imageForKey:@"hso.png"];
				closedDoor = [pss imageForKey:@"hsrc.png"];
				locked = YES;
				color = kObjectSubType_RedKey;
				break;
		...
		Layer *collisionLayer = [gameScene.castleTileMap.layers objectAtIndex:2];
		...
		if(timer > doorTimer) {
			...
			if(![[aScene player] isEntityInTileAtCoords:CGPointMake(tileLocation.x, tileLocation.y)] && 
			   ![aScene isEntityInTileAtCoords:CGPointMake(tileLocation.x, tileLocation.y)]) {
				...
				if(doorState == kDoorState_Open) {
					[sharedSoundManager playSoundWithKey:@"doorOpen" location:CGPointMake(tileLocation.x * kTile_Width, tileLocation.y * kTile_Height)];
				} else {
					[sharedSoundManager playSoundWithKey:@"doorSlam" location:CGPointMake(tileLocation.x * kTile_Width, tileLocation.y * kTile_Height)];
				}
			}
		}
	} else {
#pragma mark Locked door
		...
		if (CGRectIntersectsRect([self collisionBounds], [aScene.player movementBounds]) && doorState == kDoorState_Closed) {
			...
			// ... if they have then get the color of each key in their inventory
			int colorKey1 = -1;
			int colorKey2 = -1;
			int colorKey3 = -1;
			
			// Only set the colorKey from -1 is the inventory slot has something in it
			if (aScene.player.inventory1) {
				colorKey1 = aScene.player.inventory1.subType;
			}
			if (aScene.player.inventory2) {
				colorKey2 = aScene.player.inventory2.subType;
			}
			if (aScene.player.inventory3) {
				colorKey3 = aScene.player.inventory3.subType;
			}
			
			// If the color of the door matches the colorKey in any of the inventory slots then
			// unlock the door, set the tile to unblocked and play the door opening sound
			if (color == colorKey1 || color == colorKey2 || color == colorKey3) {
				doorState = kDoorState_Open;
				[aScene setBlocked:tileLocation.x y:tileLocation.y blocked:NO];
				[sharedSoundManager playSoundWithKey:@"doorOpen" location:CGPointMake(tileLocation.x * kTile_Width, tileLocation.y * kTile_Height)];
			}
		}
	}
